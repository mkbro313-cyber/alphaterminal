    ...
    try:
        ...
        for ticker in symbols_list:
            try:
                ...
                macd_l, macd_s, _ = calculate_macd(df, fast=12, slow=26, signal=9)
                latest_macd = float(macd_l.iloc[-1])
                latest_signal = float(macd_s.iloc[-1])
...
if st.session_state.get('data_ready', False):
    ...
    if st.session_state["view_mode"] == "deals_tracker":
        ...
        with chart_layout_right:
            ...
            with sd_col3:
                is_dark_theme = st.toggle("🌙 Dark Mode Chart", value=True, key="toggle_chart_theme_key")

            ind_col1, ind_col2 = st.columns(2)
            with ind_col1:
                enable_rsi = st.toggle("📊 RSI Indicator (14)", value=False, key="toggle_rsi_key")
            with ind_col2:
                enable_macd = st.toggle("⚡ MACD Indicator", value=False, key="toggle_macd_key")
